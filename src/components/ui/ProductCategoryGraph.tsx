...
export function ProductCategoryGraph({
    ...
    (pIdx: number | null) => {
      ...
      if (pIdx === null) {
        ...
        const catProducts = PRODUCTS.filter(
          (prod) => prod.category.id === PRODUCT_CATEGORIES[pIdx].id,
        );
  ...
  // ── Visibility-gated rAF draw loop ─────────────────────────────────────────
  useEffect(() => {
    ...
    // Pause loop when canvas is not visible (saves CPU when scrolled past hero)
    let isVisible = true;
    ...
    const frame = () => {
      ...
      if (sun) {
        ...
        if (expanded === null) {
          for (let p = 0; p < 3; p++) {
            ...
            if (!pl) continue;
            ctx.beginPath();
            ctx.arc(pl.x, pl.y, 66, 0, Math.PI * 2);
            ctx.stroke();
          }
        }
        ctx.restore();
      ...
      if (expanded !== null) {
        ...
        ctx.setLineDash([4, 8]);
        ctx.beginPath();
        ctx.arc(cx, cy, EXPAND_RING_R, 0, Math.PI * 2);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.restore();
      }
  ...
  const onMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    // Throttle to ~30fps for cursor repulsion — no visual difference, halves CPU cost
  ...
  const onClick = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (expanded !== null) {
        for (const n of nodes) {
          if (n.type !== 'moon' || n.pIdx !== expanded || n.spiralProgress < 0.3 || !n.slug)
            continue;
          // Use dynamic pill width based on label text
          const dx = Math.abs(n.x - mx);
          const dy = Math.abs(n.y - my);
          const pillCtx = canvasRef.current?.getContext('2d');
          if (pillCtx) {
            pillCtx.font = "600 8px 'Inter', sans-serif";
            const pillHalfWidth = pillCtx.measureText(n.label ?? '').width / 2 + 7 + 4; // padX + margin
            if (dx < pillHalfWidth && dy < 10) {
...
function drawMoonPill(ctx: CanvasRenderingContext2D, n: SolarNode, hovered: boolean) {
