...
export default function WhyUsSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ── 1. Label + heading — word-by-word with dramatic blur ──────────
      gsap.fromTo(labelRef.current,
        { opacity: 0, letterSpacing: '0.4em', y: 20 },
        {
          opacity: 1, letterSpacing: '0.1em', y: 0,
          duration: 0.7,
          ease: 'power3.out',
      ...
      if (headingRef.current) {
        ...
        gsap.fromTo(split.words,
          { opacity: 0, y: 100, skewY: 8, filter: 'blur(6px)' },
          {
      ...
      // ── 2. Cards — cinematic stagger with 3D rotation ─────────────────
  ...
  return (
    <section ref={sectionRef} id="why-us" aria-labelledby="why-us-heading" className="py-24 bg-white overflow-hidden">
      <div className="container mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <h2 ref={headingRef} id="why-us-heading" className="font-display text-[clamp(1.75rem,3vw,2.5rem)] font-bold text-brand-dark mb-4">
            Why Businesses Choose Us
          </h2>
          <p ref={subRef} className="text-[17px] text-slate-500 max-w-lg mx-auto" style={{ opacity: 0 }}>
        ...
        <div ref={cardsRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {REASONS.map(({ icon: Icon, title, description, iconBg, iconColor, accent }) => (
            <div key={title} className="why-card card-hover-feature bg-surface-muted rounded-[16px] p-7 border border-slate-200 h-full" style={{ opacity: 0 }}>
              <div className="why-icon w-12 h-12 rounded-[12px] flex items-center justify-center mb-5" style={{ background: iconBg, opacity: 0 }}>
                <Icon className="w-6 h-6" style={{ color: iconColor }} aria-hidden="true" />
              </div>
              <div className="accent-bar w-full h-[3px] rounded-full mb-4" style={{ background: accent }} />
