...
**Bundle baseline, `next@16.3.5`, 2026-09-26 (contact email `info@` → `connect@`):** 954.4 KB raw (**977298 B**) across **27 files**, **308.3 KB gzipped** (315649 B). Compare against this, and update it in the same commit as any deliberate change. **+3 B raw (+0.0003%)** and **0 B gzipped** against the 977295 B / 315649 B baseline immediately below it, which it supersedes: `BRAND.EMAIL` in `src/constants/index.ts:10` grew three characters, and it is emitted **once** — `Footer.tsx` and `ContactSection.tsx` both reference the identifier rather than the literal, so the constants module carries the address a single time. The file count held at 27 because the change alters a string inside an existing chunk and adds no runtime. This is the first entry in the chain where the raw figure moves and the gzipped figure does not — three bytes inside an already-compressed string literal are absorbed completely — so a reviewer who compares only the gzipped number sees an exact match and would wrongly conclude nothing shipped. Prior baseline, `next@16.3.5`, 2026-09-26 (Vercel Analytics removal): 954.4 KB raw (977295 B) across 27 files, 308.3 KB gzipped (315649 B) — **−1360 B gzipped (−0.43%)** and −3432 B raw (−0.35%) against the 317009 B / 980727 B baseline immediately below it, which it supersedes: `src/app/layout.tsx` dropped the `<Analytics />` element and its `@vercel/analytics/react` import, and `pnpm remove @vercel/analytics` took the package out of `package.json` and `pnpm-lock.yaml`. The site deploys on **Netlify** now, where the component's injected `/_vercel/insights/script.js` resolves only on Vercel's infrastructure — so these were not merely unused bytes, they were bytes buying a 404 on every page load for every visitor. The file count held at 27 because `@vercel/analytics` was bundled into the existing root-layout client chunk rather than emitting one of its own, so the removal takes bytes out of a chunk that still exists; it is the second-largest decrease recorded in this chain, behind only the −4237 B intro/`ProductCategoryGraph` pass of 2026-09-25. The same pass added `netlify.toml`, which contributed **0 B** for the ordinary reason that a repo-root config file never enters a `.next/static/**/*.js` measurement. Prior baseline, `next@16.3.5`, 2026-09-26 (homepage scroll-reset gate): 957.7 KB raw (980727 B) across 27 files, 309.6 KB gzipped (317009 B) — **−5 B gzipped (−0.002%)** and −21 B raw against the 317014 B / 980748 B baseline immediately below it, which it supersedes: `src/components/ui/LoadingScreen.tsx` dropped `className="contents"` from the wrapper that gates the homepage's children. That wrapper is the only element child of `<main>` on `/`, and a `display: contents` element generates no box at all, so `getBoundingClientRect()` returns all zeros — which Next's `shouldSkipElement` reads as "hidden", sending the router to `nextElementSibling`, finding `null`, and scrolling nowhere on every client navigation into `/`. The delta is the deleted class string and nothing else; the file count held at 27 because a removed attribute takes bytes out of an existing chunk and adds no runtime. Prior baseline, `next@16.3.5`, 2026-09-26 (mobile-nav scroll-restore guard): 957.8 KB raw (980748 B) across 27 files, 309.6 KB gzipped (317014 B) — **+24 B gzipped (+0.008%)** and +105 B raw (+0.01%) against the 316990 B / 980643 B measurement this line supersedes, recorded immediately below: the scroll lock in `src/components/layout/Header.tsx` stopped re-applying the captured offset when the close is caused by a link tap — a `navigatingRef` set by the menu links' `onClick` and consumed in the lock's cleanup — because that restore raced the router's own scroll reset and landed the visitor on the NEW route at the previous page's offset. The file count held at 27 because the change adds one ref, one callback and one guard inside an existing chunk and pulls in no new runtime import. Prior baseline, `next@16.3.5`, 2026-09-26, before that pass: 957.7 KB raw (980643 B) across 27 files, 309.6 KB gzipped (316990 B) — **+126 B gzipped (+0.04%)** and +381 B raw (+0.04%) against the 316864 B / 980262 B measurement it superseded: the scroll lock in `src/components/layout/Header.tsx` stopped setting `overflow: hidden` on the body and nothing else, and now captures `window.scrollY`, pins the body with `position: fixed`, and restores five inline properties plus the offset on release — with a one-line guard in the header's scroll listener against the clamp that pin causes. The file count held at 27 because the change adds statements inside an existing chunk and pulls in no new runtime import. Prior baseline, `next@16.3.5`, 2026-09-26, before that pass: 957.3 KB raw (980262 B) across 27 files, 309.4 KB gzipped (316864 B) — **+3618 B gzipped (+1.16%)** and +8160 B raw (+0.84%) against the 313246 B / 972102 B baseline immediately below it, and the first entry in this chain where the **file count moves** — 24 → 27. Read that file count carefully before attributing it, because it is not three new code chunks: `.next/static/chunks/` still holds exactly 24 entries (23 content-hashed plus `turbopack-*.js`), which was the whole of the previous measurement, and the three additions are the build-id manifest directory `.next/static/<buildId>/` — `_buildManifest.js`, `_clientMiddlewareManifest.js` and `_ssgManifest.js`, 423 B raw between them. They carry no application code at all (`__rewrites` and `__MIDDLEWARE_MATCHERS` are both emitted empty); they appeared in the same pass that gave `next.config.ts` an `async headers()` block, which is what makes the build emit routing metadata, though that link is inferred from the files' contents and not proven by a differential build. So the growth is byte-for-byte inside existing chunks: `src/app/error.tsx` and `src/app/global-error.tsx` (new error boundaries, necessarily client components), `src/hooks/useFocusTrap.ts` behind the header's mobile menu, and the contact form's new per-field error nodes, honeypot wrapper and `maxLength` props. The same pass's `src/app/actions/contact.ts` rewrite — per-field validation, length ceilings, the `VERCEL_ENV` sentinel guard — contributed **0 B**, for the reason already recorded twice below: a server action never enters a `.next/static/**/*.js` measurement. (Prior 2026-09-25, before this pass: 949.3 KB raw (972102 B) across 24 files, 305.9 KB gzipped (313246 B). That pass was the largest decrease recorded in this history — **−4237 B gzipped (−1.33%)** and −12383 B raw (−1.26%) against the 317483 B / 984485 B baseline immediately below it, more than triple the previous largest (−1098 B, −0.35%, the SEALED palette migration further down this chain). Two changes landed in the same pass. The intro was rewritten from six unrelated effects over 4000ms (floating particles, radial glow, a slot-machine keyword reel, a gradient sweep, a decorative progress bar, back-ease overshoot) down to one seam-and-seal gesture over ~1400ms, deleting the `float`/`gradientSweep` keyframes from `tailwind.config.ts` and the `slotMachine` keyframes and `.loading-particle` rules from `globals.css`. And `src/components/ui/ProductCategoryGraph.tsx` — 733 lines of hand-rolled Canvas 2D spring physics modelling all 50 products — was deleted outright, along with the `useState`/`useCallback`/`matchMedia` state it required in `HeroSection`, its rotating orbit-text SVG, its 14-test spec, and an `sr-only hidden md:block` `<nav>` of 50 product links. The file count held at 24 because every deletion removed code from an existing chunk rather than eliminating a chunk of its own. Notably, the additions in the same pass contributed **0 B**: a new `src/app/sitemap.ts` restores static crawlability of the 50 product pages, and it plus `HeroSection`'s static specimen-plate replacement pull in the new `SITE_URL` constant from `src/constants/index.ts` — but `SITE_URL` is reachable only from `src/app/layout.tsx` (a server component) and the sitemap route itself, so tree-shaking drops it from every client chunk even though the module holding it is otherwise client-bundled. The generalisable lesson, which surprised the implementer who predicted otherwise: a client-bundled module's every export does not necessarily reach the client — the same shape as the `flattenForSubject`/font-asset notes already below. The restored `public/hero/cup-three-quarter.webp` (84828 B) likewise contributed 0 B, for the ordinary reason that `public/` assets never enter a `.next/static/**/*.js` measurement. Prior 2026-09-25, before that pass: 960.6 KB raw (984485 B) across 24 files, 310.0 KB gzipped (317483 B). Prior 2026-09-25, before that pass: 960.1 KB raw, 309.6 KB / 316999 B gzipped — +484 B gzipped, +0.15% — the `ContactSection` `onSubmit`/`startTransition` rewrite, the "Other" free-text product branch, the `AnimateIn`/`PageTransition` SSR-reveal rework and the derived catalogue constants. The file count held at 24 because no new chunk was emitted — the change adds state and markup inside existing chunks and pulls in no new runtime import. The server-side `flattenForSubject` and phone-regex changes in `src/app/actions/contact.ts` contributed **0 B**, measured byte-identical before and after: a server-action-only edit does not enter the `.next/static/**/*.js` measurement at all. Prior 2026-09-24, before that pass: 960.0 KB raw, 309.6 KB / 316985 B gzipped — the WCAG 1.4.3 contrast pass's eight grey-token usage swaps plus five `PRODUCTS.length` / `PRODUCT_CATEGORIES.length` interpolations replacing hand-typed counts took **+14 B**, +0.004%; the file count held at 24 because the change only swaps class-string tokens and short literals inside existing chunks and adds no runtime. Note the gzipped KB figure is unchanged at 309.6 — only the raw KB moves, 960.0 → 960.1. Prior 2026-09-24, before the product-page border-token swap: 960.0 KB raw, 309.5 KB / 316976 B gzipped — changing `border-grey-100` to `border-grey-200` at `src/app/products/[slug]/page.tsx:110` took **+9 B**, +0.003%; the file count held at 24 because the change only swaps one class-string token in an existing chunk and adds no runtime. Prior 2026-09-24, before the SEALED radius/elevation/surface conformance pass: 960.1 KB raw, 309.6 KB / 317082 B gzipped — deleting 16 border-radius utilities, migrating 5 off-scale shadows onto the `e0`–`e5` scale and replacing one surface gradient with a solid fill took **−106 B**, −0.03%; the file count held at 24 because the change only removes class strings from existing chunks and adds no runtime. Prior 2026-09-24, before the SEALED palette migration: 963.0 KB raw, 310.7 KB / 318180 B gzipped — moving off the dark-navy/gradient/glass system to `ink`/`paper` took **−1098 B**, −0.35%, the first decrease recorded here. `AuroraBackground.tsx` was deleted outright, several inline style objects and gradient/blur CSS rules went with it, and no new runtime import was added; the file count held at 24 because the deletion removed markup from an existing chunk rather than a chunk of its own. Prior 2026-09-24, before the category specimen plates: 962.4 KB raw, 310.4 KB / 317896 B gzipped — swapping the emoji category icons for `next/image` panels moved it **+284 B**, +0.09%, and the file count held at 24 because `next/image` was **already** bundled by `Header.tsx`, `HeroSection.tsx` and `LoadingScreen.tsx`, so no new runtime entered; the delta is the three `PLATE_ALT` string literals, ~335 chars, plus the new JSX props. Prior 2026-09-18: 963.0 KB raw, 310.6 KB / 318067 B gzipped — the SEALED design waves' `geist` fonts and `extendTailwindMerge` moved it **−171 B**, effectively flat, because font assets are not `.js` and never enter this measurement.)
